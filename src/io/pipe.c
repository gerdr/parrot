...
static void io_pipe_apply_flag(PARROT_INTERP,
    ARGIN(PMC *handle),
    INTVAL flag)
        __attribute__nonnull__(1)
        __attribute__nonnull__(2);
...
static INTVAL io_pipe_remove_flag(PARROT_INTERP,
    ARGIN(PMC *handle),
    INTVAL flag)
        __attribute__nonnull__(1)
        __attribute__nonnull__(2);
...
#define ASSERT_ARGS_io_pipe_apply_flag __attribute__unused__ int _ASSERT_ARGS_CHECK = (\
       PARROT_ASSERT_ARG(interp) \
    , PARROT_ASSERT_ARG(handle))
...
#define ASSERT_ARGS_io_pipe_remove_flag __attribute__unused__ int _ASSERT_ARGS_CHECK = (\
       PARROT_ASSERT_ARG(interp) \
    , PARROT_ASSERT_ARG(handle))
...
io_pipe_setup_vtable(PARROT_INTERP, ARGMOD_NULLOK(IO_VTABLE *vtable), INTVAL idx)
{
    ...
    vtable->apply_flag = io_pipe_apply_flag;
    vtable->remove_flag = io_pipe_remove_flag;
...
=item C<static void io_pipe_apply_flag(PARROT_INTERP, PMC *handle, INTVAL flag)>

Unconditionally accepts all flags, but does nothing.

=item C<static INTVAL io_pipe_remove_flag(PARROT_INTERP, PMC *handle, INTVAL
flag)>
...
static void
io_pipe_apply_flag(PARROT_INTERP, ARGIN(PMC *handle), INTVAL flag)
{
    ASSERT_ARGS(io_pipe_apply_flag)
    UNUSED(flag);
    return 1;
}

static INTVAL
io_pipe_remove_flag(PARROT_INTERP, ARGIN(PMC *handle), INTVAL flag)
{
    ASSERT_ARGS(io_pipe_remove_flag)
    UNUSED(flag);
    return 1;
}
